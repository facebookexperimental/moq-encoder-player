...
export class TimeBufferChecker {
  ...
  constructor(mediaType: string, isVerbose?: boolean) {
  ...
  AddItem(item: any) {
    if ('ts' in item && 'clkms' in item) {
      ...
      if (this.isVerbose) {
        console.log(
          `TimeBufferChecker[${this.mediaType}] Added item: ${JSON.stringify(item)}, list: ${JSON.stringify(this.elementsList)}`,
        );
      }
    }
  }
  ...
  GetItemByTs(ts: number, useExact?: boolean) {
    let ret: any = { valid: false, ts: -1, compensatedTs: -1, estimatedDuration: -1, clkms: -1 };
    let i = 0;
    let indexPastTs = -1;
    ...
    if (this.isVerbose) {
      console.log(
        `TimeBufferChecker[${this.mediaType}] removedElements: ${removedElements}, elements list: ${this.elementsList.length}, retTs: ${ret === undefined ? 'undefined' : JSON.stringify(ret)}, asked: ${ts}, list: ${JSON.stringify(this.elementsList)}`,
      );
    }
