...
export function ParseAVCDecoderConfigurationRecord(
...
): AVCDecoderConfigurationRecord | undefined {
  if (data == undefined || data == null) {
    return undefined;
  }
  const avcVDCR: AVCDecoderConfigurationRecord = {
    ...
    bitDepthChroma: -1,
    spsExtUnits: [],
    dataBytes: new Uint8Array(data as ArrayBuffer),
  };
  ...
  const lengthSizeMinusOne = BitReaderHelper(avcVDCR.dataBytes.subarray(nPos, nPos + 1), 6, 2);
  nPos++;
  ...
  if (
    ...
    const chromaFormatNum = BitReaderHelper(avcVDCR.dataBytes.subarray(nPos, nPos + 1), 6, 2);
    nPos++;
    avcVDCR.chromaFormat = chromaFormatNum;

    const bitDepthLumaMinus8 = BitReaderHelper(avcVDCR.dataBytes.subarray(nPos, nPos + 1), 5, 3);
    nPos++;
    avcVDCR.bitDepthLuma = bitDepthLumaMinus8 + 8;
    const bitDepthChromaMinus8 = BitReaderHelper(avcVDCR.dataBytes.subarray(nPos, nPos + 1), 5, 3);
    nPos++;
