...
class SourceBufferProcessor extends AudioWorkletProcessor {
  ...
  process(inputs: Float32Array[][], outputs: Float32Array[][], _parameters: any) {
    ...
    if (numOutSamplesFirstChannel === undefined || numOutSamplesFirstChannel <= 0) {
      throw new Error('Num samples to process for 1st channel is not valid');
    }
    ...
    if (this.circularBufferSizeSamples <= 0) {
      throw new Error('Bad size for circular audio buffer');
    }
    ...
    if (numOutSamplesFirstChannel > this._getUsedSlots(start, end)) {
      ...
      const totalSilenceInsertedMs =
        (this.totalSilenceInsertedSamples * 1000) / this.contextSampleFrequency;
      Atomics.store(
        this.sharedStates,
      ...
      if (start + numOutSamplesFirstChannel <= this.circularBufferSizeSamples) {
        ...
        for (let c = 0; c < outputFirstTrack.length; c++) {
          const outputRingBufferPortion = new Float32Array(
            this.sharedAudiobuffers[c],
            start * Float32Array.BYTES_PER_ELEMENT,
            numOutSamplesFirstChannel,
          );
        ...
        Atomics.store(
          this.sharedStates,
          SharedStates.AUDIO_BUFF_START,
          start + numOutSamplesFirstChannel,
        );
      } else {
        ...
        for (let c = 0; c < outputFirstTrack.length; c++) {
          // First half
          const outputRingBufferPortionFirstHalf = new Float32Array(
            this.sharedAudiobuffers[c],
            start * Float32Array.BYTES_PER_ELEMENT,
            samplesToCopyFirstHalf,
          );
          outputFirstTrack[c].set(outputRingBufferPortionFirstHalf);
          // Second half
          const outputRingBufferPortionSecondHalf = new Float32Array(
            this.sharedAudiobuffers[c],
            0,
            samplesToCopySecondsHalf,
          );
...
registerProcessor('source-buffer', SourceBufferProcessor);
