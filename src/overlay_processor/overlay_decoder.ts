...
export class OverlayDecoder {
  ...
  Decode(vFrame: any) {
    ...
    const mewFramepixelsData = new Uint8Array(
      parseInt((vFrame.codedWidth * vFrame.codedHeight * (1 + 1 / 2 + 1 / 2)) as unknown as string),
    );
    ...
    const copyOptions = {
      layout: [
        ...
        {
          offset:
            vFrame.codedHeight * vFrame.codedWidth + (vFrame.codedHeight * vFrame.codedWidth) / 2,
