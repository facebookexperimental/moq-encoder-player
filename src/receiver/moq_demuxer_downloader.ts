...
let tracks: Record<string, any> = {}; // We add subscribeId and trackAlias
// Example
/* moqTracks: {
    "audio": {
      namespace: ["vc"],
...
async function moqReceiveObjects(moqt: any) {
  if (workerState === StateEnum.Stopped) {
    return;
  }
  if (moqt.wt === null) {
    sendMessageToMain(
      WORKER_PREFIX,
      'error',
      'we can not start downloading streams because WT is not initialized',
    );
    return;
  }
  ...
  try {
    ...
    sendMessageToMain(WORKER_PREFIX, 'dropped data', {
      clkms: Date.now(),
      seqId: -1,
      msg: `Dropped stream because WT error: ${err}`,
    });
    sendMessageToMain(WORKER_PREFIX, 'error', `WT request. Err: ${JSON.stringify(err)}`);
  }
}

async function moqReceiveStreamObjects(moqt: any) {
  // Get stream
    ...
    if (!stream.done) {
      ...
      if (isMoqObjectStreamHeaderType(moqStreamsObjHeader.type)) {
        sendMessageToMain(
          WORKER_PREFIX,
          'debug',
          `Received object header subgroup ${JSON.stringify(moqStreamsObjHeader)}`,
        );
        // NO await on purpose!
        ...
        sendMessageToMain(
          WORKER_PREFIX,
          'error',
          `Unsupported stream type for sterams ${moqStreamsObjHeader.type}`,
        );
      }
    }
  }
...
async function moqReceiveMultiObjectStream(readerStream: any, type: number) {
  let isEOF = false;
  let numObjRead = 0;
  let objHeader: any = {};
  while (workerState !== StateEnum.Stopped && isEOF === false) {
    ...
    try {
      ...
      sendMessageToMain(
        WORKER_PREFIX,
        'debug',
        `Received subgrp object header ${JSON.stringify(objHeader)}`,
      );

      // Check if we received the end of the subgroup
      isEOF =
        'status' in objHeader &&
        (objHeader.status == MOQ_OBJ_STATUS_END_OF_GROUP ||
          objHeader.status == MOQ_OBJ_STATUS_END_OF_TRACK_AND_GROUP ||
          objHeader.status == MOQ_OBJ_STATUS_END_OF_SUBGROUP);
      if (!isEOF && objHeader.payloadLength > 0) {
        isEOF = await readAndSendPayload(
          readerStream,
...
async function moqReceiveDatagramObjects(moqt: any) {
  ...
  while (workerState !== StateEnum.Stopped) {
    ...
    if (!stream.done) {
      ...
      sendMessageToMain(
        WORKER_PREFIX,
        'debug',
        `Received object datagram header ${JSON.stringify(moqObjHeader)}`,
      );

      if (!isMoqObjectDatagramType(moqObjHeader.type)) {
        throw new Error(
          `Received via datagram a non properly encoded object ${JSON.stringify(moqObjHeader)}`,
        );
      }
...
async function readAndSendPayload(readerStream: any, extensionHeaders: any[], length?: number) {
  ...
  self.postMessage({
    ...
    captureClkms: (chunkData as any).wallclock,
    seqId: chunkData.seqId,
    chunk,
    metadata: (chunkData as any).metadata,
    sampleFreq: (chunkData as any).sampleFreq,
    numChannels: (chunkData as any).numChannels,
  });
...
async function moqCreateSubscriberSession(moqt: any) {
  ...
  while (pending_subscribes.length > 0) {
    ...
    while (continueLoopingForAnswer) {
      const moqMsg = await moqParseMsg(moqt.controlReader);
      if (
        moqMsg.type !== MOQ_MESSAGE_SUBSCRIBE_OK &&
        moqMsg.type !== MOQ_MESSAGE_SUBSCRIBE_ERROR &&
      ...
      if (moqMsg.type === MOQ_MESSAGE_SUBSCRIBE_ERROR) {
        sendMessageToMain(
          WORKER_PREFIX,
          'warning',
          `Received SUBSCRIBE_ERROR response for ${getFullTrackName(trackData.namespace, trackData.name)} (type: ${trackType}): ${JSON.stringify(moqMsg.data)}. waiting for ${SLEEP_SUBSCRIBE_ERROR_MS}ms and Retrying!!`,
        );
        ...
        if (subscribeResp.requestId !== reqId) {
          throw new Error(
            `Received subscribeId does NOT match with subscriptionId ${subscribeResp.reqId} != ${reqId}`,
          );
        }
        sendMessageToMain(
          WORKER_PREFIX,
          'info',
          `Received SUBSCRIBE_OK for ${getFullTrackName(trackData.namespace, trackData.name)}-(type: ${trackType}): ${JSON.stringify(subscribeResp)}`,
        );
...
async function unSubscribeTracks(moqt: any) {
    ...
    if ('subscribeId' in trackData) {
      try {
        ...
        sendMessageToMain(
          WORKER_PREFIX,
          'info',
          `Received SubscribeDone for subscibeId: ${subscribeDone.subscribeId}: ${JSON.stringify(subscribeDone)}`,
        );
        if (subscribeDone.subscribeId != trackData.subscribeId) {
          throw new Error(
            `Expected MOQ_MESSAGE_SUBSCRIBE_DONE for subscribeId: ${trackData.subscribeId}, received: ${subscribeDone.subscribeId}`,
          );
        }
      } catch (err) {
        if (MOQT_DEV_MODE) {
