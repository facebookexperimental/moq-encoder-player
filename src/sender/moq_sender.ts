...
import {
  ...
  MOQ_MAPPING_SUBGROUP_PER_GROUP,
...
let moqPublisherState: Record<string, any> = {};
...
let certificateHash: any = null;

let lastObjectSentMs = 0;

let currentClientRequestId: number | undefined = undefined;
...
self.addEventListener('message', async function (e) {
  ...
  if (type === 'stop') {
    ...
    try {
      ...
      sendMessageToMain(
        WORKER_PREFIX,
        'info',
        `Aborting / closing streams while exiting. Err: ${err.message}`,
      );
    }
    return;
  }
  ...
  if (type === 'muxersendini') {
    if (workerState !== StateEnum.Instantiated) {
      sendMessageToMain(
        WORKER_PREFIX,
        'error',
        'received ini message in wrong state. State: ' + workerState,
      );
      return;
    }
    ...
    try {
      ...
      sendMessageToMain(
        WORKER_PREFIX,
        'info',
        `WT initiating with options ${JSON.stringify(options)}`,
      );
      ...
      startLoopSubscriptionsLoop(moqt.controlReader, moqt.controlWriter)
        .then(() => {
          sendMessageToMain(
            WORKER_PREFIX,
            'info',
            'Exited receiving subscription loop in control stream',
          );
        })
        .catch((err: any) => {
          if (MOQT_DEV_MODE) {
          ...
          if (workerState !== StateEnum.Stopped) {
            sendMessageToMain(
              WORKER_PREFIX,
              'error',
              `Error in the subscription loop in control stream. Err: ${JSON.stringify(err)}`,
            );
          } else {
            sendMessageToMain(
              WORKER_PREFIX,
              'info',
              `Exited receiving subscription loop in control stream. Err: ${JSON.stringify(err)}`,
            );
          }
        });
      ...
      if (keepAlivesEveryMs > 0) {
        ...
        sendMessageToMain(
          WORKER_PREFIX,
          'info',
          `Starting keep alive every ${keepAlivesEveryMs}ms, ns: ${keepAliveNameSpace}`,
        );
  ...
  if (!(type in tracks)) {
    sendMessageToMain(
      WORKER_PREFIX,
      'error',
      `Invalid message received ${type} is NOT in tracks ${JSON.stringify(tracks)}`,
    );
    return;
  }
  ...
  if (!('subscribers' in tracks[type]) || tracks[type].subscribers.length <= 0) {
    sendMessageToMain(WORKER_PREFIX, 'dropped', {
      ...
      seqId: e.data.seqId,
      ts: e.data.chunk.timestamp,
      msg: `Dropped chunk because there is NO subscribers for track ${type}`,
    });
    return;
  }
  ...
  const firstFrameClkms =
    e.data.firstFrameClkms === undefined || e.data.firstFrameClkms < 0 ? 0 : e.data.firstFrameClkms;
  const compensatedTs =
    e.data.compensatedTs === undefined || e.data.compensatedTs < 0 ? 0 : e.data.compensatedTs;
  const estimatedDuration =
    e.data.estimatedDuration === undefined || e.data.estimatedDuration < 0
      ? e.data.chunk.duration
      : e.data.estimatedDuration;
  ...
  const chunkData = {
    ...
    timebase: e.data.timebase,
    sampleFreq: e.data.sampleFreq,
    numChannels: e.data.numChannels,
    codec: e.data.codec,
    newSubgroupEvery: tracks[type].newSubgroupEvery,
  };
  ...
  while (i < tracks[type].subscribers.length) {
    ...
    sendChunkToTransport(
      chunkData,
      trackAlias,
      getNumInflightRequestByType(moqt, type),
      tracks[type].maxInFlightRequests,
      moqMapping,
    )
      .then((val: any) => {
        if (val !== undefined && val.dropped === true) {
          ...
          sendMessageToMain(
            WORKER_PREFIX,
            'debug',
            `SENT CHUNK ${type} - seqId: ${seqId}, metadataSize: ${chunkData.metadata != undefined ? chunkData.metadata.byteLength : 0} for trackAlias: ${trackAlias}`,
          );
        }
      })
      .catch((err: any) => {
        ...
        sendMessageToMain(
          WORKER_PREFIX,
          'error',
          `error sending chunk. For trackAlias: ${trackAlias}. Err:  ${err.message}`,
        );
      });
    i++;
  }
  ...
  if (isSendingStats) {
    self.postMessage({
      type: 'sendstats',
      clkms: Date.now(),
      inFlightReq: getInflightRequestsReport(moqt),
    });
  }
});

async function sendKeepAlive(controlWriter: any) {
  if (Date.now() - lastObjectSentMs > keepAlivesEveryMs) {
    ...
    sendMessageToMain(
      WORKER_PREFIX,
      'info',
      `Sent keep alive (publish) for ns: ${keepAliveNameSpace} name: ${keepAliveName}`,
    );
  }
}

async function startLoopSubscriptionsLoop(controlReader: any, controlWriter: any) {
  ...
  while (workerState === StateEnum.Running) {
    ...
    if (moqMsg.type === MOQ_MESSAGE_SUBSCRIBE_UPDATE) {
      ...
      sendMessageToMain(
        WORKER_PREFIX,
        'info',
        `Received MOQ_MESSAGE_SUBSCRIBE_UPDATE: ${JSON.stringify(subscribeUpdate)}`,
      );
      if (!('subscriptionRequestId' in subscribeUpdate) || !('forward' in subscribeUpdate)) {
        sendMessageToMain(
          WORKER_PREFIX,
          'error',
          'Invalid MOQ_MESSAGE_SUBSCRIBE_UPDATE received, subscriptionRequestId or forward not found',
        );
        continue;
      }
      ...
      if (track == null || track == undefined) {
        sendMessageToMain(
          WORKER_PREFIX,
          'error',
          `Invalid MOQ_MESSAGE_SUBSCRIBE_UPDATE could not find a track from ${publisherRequestId}. Tracks data: ${JSON.stringify(tracks)}`,
        );
        continue;
      }
      if (track.authInfo != undefined && track.authInfo != '') {
        ...
        if (track.authInfo !== authInfo) {
          const _errorCode = MOQ_SUBSCRIPTION_ERROR_INTERNAL;
          const errReason = `Invalid MOQ_MESSAGE_SUBSCRIBE_UPDATE authInfo ${authInfo}`;
          sendMessageToMain(
            WORKER_PREFIX,
            'error',
            `${errReason} does not match with ${JSON.stringify(tracks)}`,
          );
          continue;
        }
      }
      ...
      if (subscribeUpdate.forward === 1) {
        addSubscriberToTrack(
          track,
          subscribeUpdate.publisherRequestId,
          1,
        ...
        sendMessageToMain(
          WORKER_PREFIX,
          'info',
          `NOT added (forward == 0) subscriber for track ${track.trackAlias}(${track.namespace}/${track.trackName}). Current num subscriber: ${track.subscribers.length}. AuthInfo MATCHED!`,
        );
      }
      ...
      sendMessageToMain(
        WORKER_PREFIX,
        'info',
        `Received MOQ_MESSAGE_UNSUBSCRIBE: ${JSON.stringify(unsubscribe)}`,
      );
      if (!('subscriptionRequestId' in unsubscribe)) {
        sendMessageToMain(
          WORKER_PREFIX,
          'error',
          'Invalid MOQ_MESSAGE_UNSUBSCRIBE received, subscriptionRequestId not found',
        );
        continue;
      }
      ...
      if (subscribers.length > 0) {
        sendMessageToMain(
          WORKER_PREFIX,
          'info',
          `Removed subscriber(s) for subscriptionRequestId: ${JSON.stringify(subscribers)}`,
        );
      } else {
        sendMessageToMain(
          WORKER_PREFIX,
          'error',
          `Removing subscriber. Could not find subscribeId: ${unsubscribe.subscriptionRequestId}`,
        );
      }
      ...
      if (trackData == null) {
        sendMessageToMain(
          WORKER_PREFIX,
          'error',
          `Invalid subscribe received ${fullTrackName} is NOT in tracks ${JSON.stringify(trackData)}`,
        );
        continue;
      }
      if (trackData.authInfo != undefined && trackData.authInfo != '') {
        ...
        if (trackData.authInfo !== authInfo) {
          ...
          sendMessageToMain(
            WORKER_PREFIX,
            'error',
            `${errReason} does not match with ${JSON.stringify(trackData)}`,
          );
      ...
      await moqSendSubscribeOk(
        controlWriter,
        subscribe.requestId,
        trackData.trackAlias,
        0,
        lastSent.group,
        lastSent.obj,
        subscribe.parameters.authInfo,
      );
      sendMessageToMain(
        WORKER_PREFIX,
        'info',
        `Sent SUBSCRIBE_OK for requestId: ${subscribe.requestId}, last: ${lastSent.group}/${lastSent.obj}`,
      );
      ...
      sendMessageToMain(
        WORKER_PREFIX,
        'warning',
        `Unexpected message (type ${moqMsg.type} received, ignoring`,
      );
    }
  }
}

async function sendChunkToTransport(
  chunkData: any,
  trackAlias: number,
  numInflightReqType: number,
  maxFlightRequests: number,
  moqMapping: string,
) {
  if (chunkData == null) {
...
async function createRequest(chunkData: any, trackAlias: number, moqMapping: string) {
  let packet = null;
  let isHiPri = false;
  ...
  if (chunkData.mediaType === 'video') {
    ...
    const avcDecoderConfigurationRecord =
      'metadata' in chunkData && chunkData.metadata != undefined && chunkData.metadata != null
        ? chunkData.metadata
        : undefined;
...
async function createSendPromise(
  packet: MIPackager,
  trackAlias: number,
  moqMapping: string,
  isHiPri: boolean,
  ...
  if (moqt.wt === null) {
    return {
      dropped: true,
      message: `Dropped Object for trackAlias: ${trackAlias}, because transport is NOT open. SeqId: ${packet.GetData().seqId}`,
    };
  }
  ...
  const publisherPriority = isHiPri
  ...
  if (moqMapping === MOQ_MAPPING_OBJECT_PER_DATAGRAM) {
    ...
    sendMessageToMain(
      WORKER_PREFIX,
      'debug',
      `Sending Object per datagram. trackAlias: ${trackAlias} ${groupSeq}/${objSeq}(${sendOrder}). Data: ${packet.GetDataStr()}, Ext Headers: ${JSON.stringify(packet.ExtensionHeaders())}`,
    );
    ...
    if (currentUniStreamWritter == undefined) {
      ...
      sendMessageToMain(
        WORKER_PREFIX,
        'debug',
        `Created new subgroup (stream) ${currentStreamWriterId} with sendOrder: ${sendOrder}`,
      );
    ...
    if (prevGroupSeq != groupSeq) {
      ...
      const prevUniStreamWritter = moqt.multiObjectWritter[prevStreamWriterId];
      if (prevUniStreamWritter != undefined) {
        ...
        sendMessageToMain(
          WORKER_PREFIX,
          'debug',
          `Send group close for ${prevStreamWriterId} and ${prevGroupSeq}`,
        );
...
function createMultiObjectHash(mediaType: string, trackAlias: number, groupId: number) {
  return `${mediaType}-${trackAlias}-${groupId}`;
}

// MOQT

async function moqCreatePublisherSession(moqt: any, usePublishNamespace: boolean) {
  // SETUP
  ...
  sendMessageToMain(
    WORKER_PREFIX,
    'info',
    `Received MOQ_MESSAGE_SERVER_SETUP: ${JSON.stringify(moqMsg)}`,
  );
  ...
  if (usePublishNamespace) {
    ...
    for (const [, trackData] of Object.entries(tracks)) {
      ...
      if (!publishedNamespacesStr.includes(ns_str)) {
        ...
        sendMessageToMain(
          WORKER_PREFIX,
          'info',
          `Sent MOQ_MESSAGE_PUBLISH_NAMESPACE (${publishReqId}) ${trackData.namespace}, authinfo: ${JSON.stringify(trackData.authInfo)}`,
        );
        ...
        while (continueLoopingForAnswer) {
          const moqMsg = await moqParseMsg(moqt.controlReader);
          if (
            moqMsg.type !== MOQ_MESSAGE_PUBLISH_NAMESPACE_OK &&
            moqMsg.type !== MOQ_MESSAGE_PUBLISH_NAMESPACE_ERROR &&
            moqMsg.type != MOQ_MESSAGE_MAX_REQUEST_ID &&
            moqMsg.type != MOQ_MESSAGE_SUBSCRIBE
          ) {
            throw new Error(
              `Expected MOQ_MESSAGE_PUBLISH_NAMESPACE_OK or MOQ_MESSAGE_PUBLISH_NAMESPACE_ERROR or MOQ_MESSAGE_MAX_REQUEST_ID or MOQ_MESSAGE_SUBSCRIBE, received ${moqMsg.type}`,
            );
          }
          if (moqMsg.type === MOQ_MESSAGE_PUBLISH_NAMESPACE_ERROR) {
            throw new Error(
              `Received MOQ_MESSAGE_PUBLISH_NAMESPACE_ERROR response for ${trackData.namespace}/${trackData.name}: ${JSON.stringify(moqMsg.data)}`,
            );
            ...
            sendMessageToMain(
              WORKER_PREFIX,
              'info',
              `Received MOQ_MESSAGE_PUBLISH_NAMESPACE_OK response for (${publishReqId}) ${trackData.namespace}: ${JSON.stringify(reqResp)}`,
            );
            if (publishReqId != reqResp.reqId) {
              throw new Error(
                `Received RequestID ${reqResp.reqId} does NOT match with the one sent in MOQ_MESSAGE_PUBLISH ${publishReqId}`,
              );
            }
            ...
            sendMessageToMain(
              WORKER_PREFIX,
              'warning',
              `UNKNOWN message received ${JSON.stringify(moqMsg)}`,
            );
          }
        }
      }
    ...
    for (const [trackType, trackData] of Object.entries(tracks)) {
      ...
      while (continueLoopingForAnswer) {
        const moqMsg = await moqParseMsg(moqt.controlReader);
        if (
          moqMsg.type !== MOQ_MESSAGE_PUBLISH_OK &&
          moqMsg.type !== MOQ_MESSAGE_PUBLISH_ERROR &&
          moqMsg.type != MOQ_MESSAGE_MAX_REQUEST_ID
        ) {
          throw new Error(
            `Expected MOQ_MESSAGE_PUBLISH_OK or MOQ_MESSAGE_PUBLISH_ERROR or MOQ_MESSAGE_MAX_REQUEST_ID, received ${moqMsg.type}`,
          );
        }
        if (moqMsg.type === MOQ_MESSAGE_PUBLISH_ERROR) {
          throw new Error(
            `Received MOQ_MESSAGE_PUBLISH_ERROR response for ${trackData.namespace}/${trackData.name}-(type: ${trackType}): ${JSON.stringify(moqMsg.data)}`,
          );
        } else if (moqMsg.type === MOQ_MESSAGE_PUBLISH_OK) {
          ...
          sendMessageToMain(
            WORKER_PREFIX,
            'info',
            `Received MOQ_MESSAGE_PUBLISH_OK response for ${trackData.id}-${trackType}-${trackData.namespace}: ${JSON.stringify(publishResp)}`,
          );
          if (publishReqId != publishResp.reqId) {
            throw new Error(
              `Received RequestID ${publishResp.reqId} does NOT match with the one sent in MOQ_MESSAGE_PUBLISH ${publishReqId}`,
            );
          }
...
function moqCalculateSendOrder(packet: MIPackager, isHiPri: boolean) {
  // Prioritize:
  // Audio over video
  // New over old
...
function getTrackFromPublishRequestId(reqId: number) {
  for (const [, trackData] of Object.entries(tracks)) {
...
function removeSubscriberFromTrackByPublisherId(requestId: number) {
  const ret = [];
...
function getNumInflightRequestByType(moqt: any, trackType: string) {
  let ret = 0;
...
function getInflightRequestsReport(moqt: any) {
  const ret: any = {};
...
function getLastSentFromTrackAlias(trackAlias: number) {
  const ret: any = { group: undefined, obj: undefined };
  if (trackAlias in moqPublisherState) {
...
async function sendPublishDone(moqt: any) {
  for (const trackData of Object.values(tracks)) {
    try {
      if ('publisherRequestId' in trackData) {
        ...
        sendMessageToMain(
          WORKER_PREFIX,
          'info',
          `Sent PUBLISH_DONE for ${JSON.stringify(trackData)}`,
        );
      }
    } catch (err) {
      if (MOQT_DEV_MODE) {
...
function createSubscribeUpdate(
  requestId: number,
...
function addSubscriberToTrack(
  ...
  sendMessageToMain(
    WORKER_PREFIX,
    'info',
    `New subscriber for track ${trackData.trackAlias}(${trackData.namespace}/${trackData.trackName}). Current num subscriber: ${trackData.subscribers.length}`,
  );
}

function stringifyNamespace(ns: string[]) {
  return ns.join('-');
}

function getTrackFromFullTrackName(fullTrackName: string) {
  for (const [, trackData] of Object.entries(tracks)) {
