...
function setWaitForKeyframe(value: boolean) {
  ...
  if (type === 'stop') {
    ...
    if (videoDecoder.decodeQueueSize >= maxQueuedChunks) {
      ...
      sendMessageToMain(
        WORKER_PREFIX,
        'warning',
        'Discarded ' + discardedBufferFull + ' video chunks because decoder buffer is full',
      );
      return;
    }
    ...
    if (isWaitingForKeyframe() && e.data.chunk.type !== 'key') {
      ...
      if (discardedDelta > 0) {
        sendMessageToMain(
          WORKER_PREFIX,
          'warning',
          'Discarded ' + discardedDelta + ' video chunks before key',
        );
      }
      discardedDelta = 0;
