...
export function concatBuffer(arr: any[]): Uint8Array {
  let totalLength = 0;
  arr.forEach((element) => {
...
export async function readUntilEof(
  ...
  while (true) {
    let bufferChunk: any = new Uint8Array(blockSize);
...
export async function buffRead(
  readableStream: ReadableStream,
  size: number,
  ...
  const buff = new Uint8Array(Number(size));
  const reader = readableStream.getReader({ mode: 'byob' });
...
export function getArrayBufferByteLength(arrayBuffer: any[]): number {
  let ret = 0;
