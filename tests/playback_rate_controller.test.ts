...
// Defaults: onTargetPerct 0.2, speedUp 1.1, slowDown 0.9. With target 200ms the
...
describe('PlaybackRateController hysteresis', () => {
  ...
  it('speeds up when over the band and holds until the buffer drains to target', () => {
    const c = mk();
    expect(c.currentRenderBuffer(241)).toBe(1.1); // crossed upper -> speed up
    expect(c.getSpeed()).toBe(1.1);
  ...
  it('slows down when under the band and holds until the buffer refills to target', () => {
    const c = mk();
    expect(c.currentRenderBuffer(159)).toBe(0.9); // crossed lower -> slow down
    expect(c.getSpeed()).toBe(0.9);
...
describe('PlaybackRateController updateConfig', () => {
  it('applies a partial update and recomputes the band', () => {
    const c = mk();
    expect(c.currentRenderBuffer(250)).toBe(1.1); // 250 > 240 with perct 0.2
  ...
  it('rejects invalid config atomically (leaves state unchanged)', () => {
    ...
    expect(c.currentRenderBuffer(150)).toBe(0.9); // 150 < 160 (old lower), not < 80
  });
});
